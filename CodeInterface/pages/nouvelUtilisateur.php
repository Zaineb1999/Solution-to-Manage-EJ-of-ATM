...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($validationErrors)) {
        if (rechercher_par_login($login) == 0 & rechercher_par_email($email) == 0) {
            $requete = 'insert into utilisateur(login,email,role,etat,pwd) 
                                        values("'.$login.'","'.$email.'","SUPERVISEUR",0,md5("'.$pwd1.'"))';

            $resultat=mysqli_query($con,$requete);
            if($resultat){

            $success_msg = "Félicitation, votre compte est crée, mais temporairement inactif jusqu'à activation par l'admin";}
        } else {
